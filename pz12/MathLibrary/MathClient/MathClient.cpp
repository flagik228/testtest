// MathClient.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//

#include <iostream>
#include "../MathLibrary/MathLib.h"

int main()
{
    setlocale(LC_ALL, "Russian");
    double a, b;
    std::cout << "Введите два числа: ";
    std::cin >> a >> b;

    double result = MathLibrary::Arithmetic::Add(a, b);
    std::cout << "Сумма: " << result << std::endl;

    result = MathLibrary::Arithmetic::Subtract(a, b);
    std::cout << "Разность: " << result << std::endl;

    result = MathLibrary::Arithmetic::Multiply(a, b);
    std::cout << "Произведение: " << result << std::endl;

    result = MathLibrary::Arithmetic::Divide(a, b);
    std::cout << "Деление: " << result << std::endl;

    result = MathLibrary::Arithmetic::Stepen(a, b);
    std::cout << "Возведение в степень: " << result << std::endl;

    result = MathLibrary::Arithmetic::Koren(a, b);
    std::cout << "Получение корня числа a: " << result << std::endl;

    result = MathLibrary::Arithmetic::Koren(b, a);
    std::cout << "Получение корня числа b: " << result << std::endl;

    return 0;
}

// Запуск программы: CTRL+F5 или меню "Отладка" > "Запуск без отладки"
// Отладка программы: F5 или меню "Отладка" > "Запустить отладку"

// Советы по началу работы 
//   1. В окне обозревателя решений можно добавлять файлы и управлять ими.
//   2. В окне Team Explorer можно подключиться к системе управления версиями.
//   3. В окне "Выходные данные" можно просматривать выходные данные сборки и другие сообщения.
//   4. В окне "Список ошибок" можно просматривать ошибки.
//   5. Последовательно выберите пункты меню "Проект" > "Добавить новый элемент", чтобы создать файлы кода, или "Проект" > "Добавить существующий элемент", чтобы добавить в проект существующие файлы кода.
//   6. Чтобы снова открыть этот проект позже, выберите пункты меню "Файл" > "Открыть" > "Проект" и выберите SLN-файл.
